...
impl<I: EventHandler + IoRead> IoReadStream for IoReader<I> {
    fn pipe<W>(self, write: W) where W: IoWriteStream {
        ...
        let on_read = move |&mut: io: &mut I, _| {
            into_continue(read_to(io, unsafe { read_handle.borrow_mut() }))
        };

        let on_write = move |&mut: writer: &mut <W as IoWriteStream>::Writer| {
            into_continue(write_from(unsafe { write_handle.borrow_mut() }, writer))
        };

        Registration::new(self.io, on_read, |&mut: _: &mut I| incorrect_writable()).register();
...
impl<I: EventHandler + IoWrite> IoWriteStream for IoWriter<I> {
    ...
    fn on_write<W: FnMut(&mut WriterTo<I>) -> bool + 'static>(self, listener: W) {
        let on_write = move |&mut: io: &mut I| {
            let writer = WriterTo { io: io };
            listener(writer)
        };

        Registration::new(self.io, |&mut: _: &mut I, _| incorrect_readable(), on_write).register();
    }
}
...
impl<I: IoWrite> FnMut(&[u8]) -> AioResult<usize> for WriterTo<I> {
